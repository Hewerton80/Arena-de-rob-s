...
class RoboTipo2TK(ItemTK,RoboTipo2):
	...
	vertices = [(l/2 , 0),(-l/2 , -l/2),(-l/2 ,l/2)]
	...
	def __init__(self,nome,x,y,ori):
		...
		x = float(x) if x else uniform(RoboTipo2TK.l, 640 - RoboTipo2TK.l)
		y = float(y) if y else uniform(RoboTipo2TK.l, 410 - RoboTipo2TK.l)
		ori = float(ori) if ori else uniform(0,360)
		pos0 = Ponto2D(x, y)
		pol = Poligono(RoboTipo2TK.vertices, pos0, ori )
	...
	def _inicializa_forma(self): 
		...
		self.item_canvas = self.arena.create_polygon((p1,p2,p3) , fill=self.cor,tag='robo', outline='#000000')
